...
def encrypt(letter, offset):
    alf = "абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯА"
    if letter in alf:
        return alf[alf.index(letter) - offset]
    else:
        return letter
str_n = 1
str_enc = ""
f = open("message.txt.TXT", "rt", encoding = "utf-8")
for i in f:
    count = 1
    while count <= len(i):
        str_enc += encrypt(i[count - 1], str_n)
        count += 1
print(str_enc)
str_n += 1
f.close()